...
def load_or_create_sequences(config: Config) -> dict:
    ...
    if config.sequences_path.exists():
        ...
        dataset = MyDataset(data_path=config.data_path, country=COUNTRY, api_key=API_KEY)
        dataset.preprocess()
...
def main():
    ...
    # Create model
    model = Model(n_features=config.n_features, n_timesteps=config.n_timesteps, n_outputs=config.n_outputs).to(
        config.device
    )
