...
from config import N_INPUT_TIMESTEPS, N_OUTPUT_TIMESTEPS, TRAIN_SIZE, VAL_SIZE, TEST_SIZE
...
class MyDataset(Dataset):
    ...
    def _split_data(
        self, train_size: float = TRAIN_SIZE, val_size: float = VAL_SIZE, test_size: float = TEST_SIZE
    ) -> None:
        """
        ...
        self.train_val_data, self.test_data = train_test_split(self.processed_data, test_size=test_size, shuffle=False)

        # Second split: train vs val
        val_ratio = val_size / (train_size + val_size)
        self.train_data, self.val_data = train_test_split(self.train_val_data, test_size=val_ratio, shuffle=False)
    ...
    def _create_sequences(self, data: np.ndarray) -> tuple:
        ...
        for i in range(len(data) - self.n_input_timesteps - self.n_output_timesteps + 1):
            ...
            y = data[
                i + self.n_input_timesteps : i + self.n_input_timesteps + self.n_output_timesteps, 0
